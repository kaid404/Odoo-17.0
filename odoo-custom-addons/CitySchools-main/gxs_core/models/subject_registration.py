...
class OpSubjectRegistration(models.Model):
    ...
    class_id = fields.Many2one('op.academic.year','Academic Class')
    ...
    def action_approve(self):
        for record in self:
            ...
            if not course_id:
                course_id = self.env['op.student.course'].create({
                    'student_id':record.student_id.id,
                    'course_id':record.course_id.id,
                    'academic_term_id':record.student_id.class_id.id,
                    'academic_years_id':record.student_id.year_id.id,
                })
            if course_id:
                course_id.write({
