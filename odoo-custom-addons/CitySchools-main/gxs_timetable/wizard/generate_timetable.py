...
class GenerateSession(models.TransientModel):
    ...
    # is_copy = fields.Boolean(string="Copy",compute='copy_timetable',readonly=False,store=True)
    ...
    def copy_timetable(self):
        ...
        for rec in self:
            for d in ['1','2','3','4','5','6']:
                for days in rec.time_table_lines_1:
                    # print(days.copy())
                    print(rec.id,'ggggggggg',days)
                    ff = days.copy()
                    print(ff)
                    ff.gen_time_table = rec.id
                    ff.day = d
                print(rec.time_table_lines_2)
                print(rec.time_table_lines_1)
        self.env.cr.commit()
        # return True
        # raise ValidationError('Coped')
    ...
    def act_gen_time_table(self):
        ...
        year_id = self.env['op.academic.term'].search([('current_year','=',True)])
