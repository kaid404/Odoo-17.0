...
class SchoolLeavingCertificate(models.Model):
    ...
    father_name = fields.Char(related='student_id.father_name',string="Father",store=True)
    term_id = fields.Many2one('op.academic.term', string='Term', store=True)
    ...
    def birth_date_to_words(self, birth_date):
        ...
        if birth_date:
            try:
                ...
                month_in_words = calendar.month_name[birth_month]
                year_in_words = num2words(birth_year, lang='en').replace(" and ", " ").capitalize()
